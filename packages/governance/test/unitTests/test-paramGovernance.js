...
import { setupGovernance, paramChangePositions } from '../../src/governParam';
import {
...
test('governParam happy path with fakes', async t => {
  ...
  const paramSpec = { parameterName: MALLEABLE_NUMBER };
  const { positive } = paramChangePositions(paramSpec.parameterName, 25n);
  ...
  const questionPoser = Far('poser', {
    addQuestion: () => {
      return { publicFacet: fakeCounterPublic };
    },
  });
...
test('governParam bad update', async t => {
  ...
  t.deepEqual(details, fakeDetails);
